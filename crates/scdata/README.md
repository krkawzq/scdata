...
`scdata` compiles an ordered list of `(source, row)` accesses into immutable,
...
```sh
cargo run --release -p scdata --features profile --example end_to_end_bench -- access ...
```
...
and session state. The access benchmark also compiles without the feature to
provide an instrumentation-free control build; only the `profile` variant can
read or print detailed counters.
