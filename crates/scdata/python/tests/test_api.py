...
def test_register_feature_map_plan_and_owned_batches(tmp_path: Path) -> None:
    ...
    dataset = scdata.register(path, key="X", feature_map=[2, None, 0])
    ...
    with plan.open(scdata.SessionConfig(worker_count=1, io_mode="blocking")) as session:
...
def test_prefetch_is_reusable_via_plan_and_preserves_row_order(tmp_path: Path) -> None:
    ...
    dataset = scdata.register(write_scc_x(tmp_path, values))
    plan = scdata.compile(
        dataset,
    ...
    config = scdata.SessionConfig(worker_count=1, io_mode="blocking")
    np.testing.assert_array_equal(plan.read(config), expected)
    result = np.concatenate(list(scdata.prefetch(dataset, [4, 0, 2], batch_size=1, prefetch_step=2, config=config)))
...
def test_cancel_is_structured_and_close_is_idempotent(tmp_path: Path) -> None:
    ...
    plan = scdata.compile(
        dataset,
        range(16),
        output=scdata.OutputSpec(4, np.float32),
        batch_size=2,
        prefetch_step=3,
    )
    session = plan.open(scdata.SessionConfig(worker_count=1, io_mode="blocking"))
    session.cancel()
    with pytest.raises(scdata.CancelledError) as raised:
        session.next_batch()
...
def test_empty_plan_finishes_without_batches() -> None:
    plan = scdata.compile(
        [],
        [],
        output=scdata.OutputSpec(3, np.float32),
        batch_size=4,
        prefetch_step=2,
    )
...
def test_checked_conversion_policies(tmp_path: Path) -> None:
    ...
    dataset = scdata.register(write_scc(adata, tmp_path / "signed.scc", store="dir"))

    failing = scdata.compile(
        dataset,
        [0],
        output=scdata.OutputSpec(2, np.uint16),
        batch_size=1,
        prefetch_step=2,
    )
    with failing.open(scdata.SessionConfig(worker_count=1, io_mode="blocking")) as session:
        with pytest.raises(scdata.SessionError):
            session.next_batch()

    filled = scdata.compile(
        dataset,
        [0],
        output=scdata.OutputSpec(2, np.uint16, fill=99, overflow="use_fill"),
        batch_size=1,
        prefetch_step=2,
    )
    with filled.open(scdata.SessionConfig(worker_count=1, io_mode="blocking")) as session:
...
def test_rounding_conversion_requires_opt_in(tmp_path: Path) -> None:
    ...
    dataset = scdata.register(write_scc(adata, tmp_path / "int32.scc", store="dir"))
    common = dict(datasets=dataset, rows=[0], batch_size=1, prefetch_step=2)
    with pytest.raises(scdata.PromotionError):
        scdata.compile(**common, output=scdata.OutputSpec(1, np.float32))

    plan = scdata.compile(
        **common,
        output=scdata.OutputSpec(1, np.float32, allow_float_rounding=True),
    )
    with plan.open(scdata.SessionConfig(worker_count=1, io_mode="blocking")) as session:
...
def test_scc_zip_and_layer_key(tmp_path: Path) -> None:
    ...
    x = scdata.register(archive, key="X")
    assert x.zip_prefix == "X"
    assert x.feature_names == ("g0", "g1", "g2")
    counts = scdata.register(archive, key="layers/counts")
    assert counts.zip_prefix == "layers/counts"

    config = scdata.SessionConfig(worker_count=1, io_mode="auto")
    np.testing.assert_array_equal(
        scdata.compile(x, [1], batch_size=1, prefetch_step=2).read(config),
        values[1:2],
    )
    np.testing.assert_array_equal(
        scdata.compile(counts, [0], batch_size=1, prefetch_step=2).read(config),
        layer[0:1],
    )
...
def test_obsm_has_no_feature_names_and_identity_prefetch(tmp_path: Path) -> None:
    ...
    pca = scdata.register(path, key="obsm/X_pca")
    assert pca.feature_names is None
    assert pca.shape == (3, 2)
    result = list(scdata.prefetch(pca, [2, 0], batch_size=1, prefetch_step=2))
...
def test_dense_and_csr_sources_can_be_interleaved(tmp_path: Path) -> None:
    ...
    dense = scdata.register(write_scc_x(tmp_path, dense_values, "dense.scc"))
    ...
    csr = scdata.register(write_scc(csr_adata, tmp_path / "csr.scc", store="dir"))
    assert csr.kind == "csr"
    ...
    with plan.open(scdata.SessionConfig(worker_count=2, io_mode="blocking")) as session:
        np.testing.assert_array_equal(
            session.read(),
...
def test_register_accepts_sc_compress_read_limits(tmp_path: Path) -> None:
    ...
    values = np.arange(6, dtype=np.float32).reshape(2, 3)
    path = write_scc_x(tmp_path, values)
    dataset = scdata.register(path, limits=scc.ReadLimits(max_block_count=10_000))
...
def test_with_feature_map_returns_new_handle(tmp_path: Path) -> None:
    values = np.arange(6, dtype=np.float32).reshape(2, 3)
    base = scdata.register(write_scc_x(tmp_path, values))
