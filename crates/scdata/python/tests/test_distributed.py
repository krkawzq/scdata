...
import scdata
import scdata.distributed as distributed_module

pytestmark = pytest.mark.skipif(
    not hasattr(scdata._core, "_attach_shared"),
...
def _consume_rank(
    iterator: scdata.DistributedIterator,
...
def _close_inherited_session_then_consume(
    distributed: scdata.DistributedSession,
    iterator: scdata.DistributedIterator,
    queue: Any,
) -> None:
    distributed.close()
...
def _plan(tmp_path: Path, rows: int = 8) -> tuple[scdata.Plan, np.ndarray]:
    values = np.arange(rows * 3, dtype=np.float32).reshape(rows, 3)
    dataset = scdata.register(_write_matrix(tmp_path, values))
    plan = scdata.compile(
        dataset,
        range(rows),
        batch_size=2,
...
def test_distributed_rank_iterators_preserve_round_robin_batches(tmp_path: Path) -> None:
    ...
    config = scdata.SessionConfig(worker_count=2, io_mode="blocking")
...
def test_distributed_copy_returns_compact_numpy_owned_batches(
    ...
    plan = scdata.compile(
        dataset,
        range(4),
        output=scdata.OutputSpec(3, dtype),
        batch_size=2,
        prefetch_step=2,
    )
    config = scdata.SessionConfig(worker_count=1, io_mode="blocking")
...
def test_distributed_copy_supports_zero_width_output(tmp_path: Path) -> None:
    ...
    dataset = scdata.register(_write_matrix(tmp_path, values)).with_feature_map([None] * 3)
    plan = scdata.compile(
        dataset,
        range(2),
        output=scdata.OutputSpec(0, np.float32),
        batch_size=2,
        prefetch_step=2,
    )
    config = scdata.SessionConfig(worker_count=1, io_mode="blocking")
...
def test_distributed_validation_and_cancellation(tmp_path: Path) -> None:
    ...
    with pytest.raises(scdata.ResourceLimitError):
    ...
    first.close()
    with pytest.raises(scdata.CancelledError):
        distributed.wait(timeout=5)
    distributed.close()
...
def test_distributed_ranks_rolls_back_partial_creation(
    ...
    original = scdata.DistributedSession._new_iterator
    created: list[scdata.DistributedIterator] = []

    def fail_second(
        session: scdata.DistributedSession,
        rank: int,
        copy: bool,
    ) -> scdata.DistributedIterator:
        if rank == 1:
    ...
    monkeypatch.setattr(scdata.DistributedSession, "_new_iterator", fail_second)
...
def test_distributed_iterator_state_restore_closes_fd_on_failure() -> None:
    ...
    iterator = scdata.DistributedIterator.__new__(scdata.DistributedIterator)
    try:
...
def test_distributed_iterator_close_releases_every_resource_after_error() -> None:
    ...
    iterator = scdata.DistributedIterator(read_fd, 0, metadata, copy=True)
...
def test_distributed_iterator_rejects_mismatched_descriptor_metadata(
    ...
    monkeypatch.setattr(scdata._core, "_attach_shared", lambda _fd, _rank: client)
    try:
...
def test_distributed_iterator_serializes_concurrent_first_consumers(
    ...
    config = scdata.SessionConfig(worker_count=1, io_mode="blocking")
    original = scdata._core._attach_shared
    ...
    monkeypatch.setattr(scdata._core, "_attach_shared", delayed_attach)
    with plan.open_distributed(1, config) as distributed:
        iterator = distributed.rank(0)
...
def test_distributed_zero_copy_reports_held_ring_slot(tmp_path: Path) -> None:
    ...
    dataset = scdata.register(_write_matrix(tmp_path, values))
    plan = scdata.compile(dataset, range(6), batch_size=2, prefetch_step=2)
    config = scdata.SessionConfig(worker_count=1, io_mode="blocking")
    with plan.open_distributed(1, config) as distributed:
        iterator = distributed.rank(0, copy=False)
        ...
        with pytest.raises(scdata.InvalidInputError, match="still holds logical batch"):
            iterator.next_batch()
        del first
        del second
...
def test_distributed_iterator_close_wakes_a_concurrent_next(tmp_path: Path) -> None:
    ...
    assert len(errors) == 1
    assert isinstance(errors[0], scdata.CancelledError)
    with pytest.raises(scdata.CancelledError):
        distributed.wait(timeout=5)
    distributed.close()
...
def test_empty_distributed_plan_finishes_for_every_rank() -> None:
    ...
    with plan.open_distributed(
        3,
        scdata.SessionConfig(worker_count=1, io_mode="blocking"),
    ) as distributed:
