...
def make_plan(root: Path, args: argparse.Namespace) -> scdata.Plan:
    ...
    dataset = scdata.register(path)
    return scdata.compile(
        dataset,
        range(args.rows),
...
def consume_read(iterator: scdata.DistributedIterator) -> tuple[int, float]:
    values = iterator.read()
    checksum = float(values[0, 0]) if values.size else 0.0
    return values.shape[0], checksum


def consume_legacy_read(iterator: scdata.DistributedIterator) -> tuple[int, float]:
...
def run_once(
    ...
    config = scdata.SessionConfig(worker_count=args.world_size, io_mode="blocking")
...
def main() -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="scdata-distributed-benchmark-") as temporary:
