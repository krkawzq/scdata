...
pub fn promote_kind(src: StorageDType, dst: OutputDType) -> Option<PromoteKind> {
    ...
    // Integer same-signedness widen / u16→i32 always in range
    if matches!(
        (src, dst),
        (S::I16, O::I32) | (S::U16, O::U32) | (S::U16, O::I32)
    ) {
        return Some(PromoteKind::Lossless);
    }
    ...
    // Every 16-bit integer is exactly representable by f32. Every supported
    ...
    // f32 has only 24 bits of integer precision.
    if matches!((src, dst), (S::I32 | S::U32, O::F32)) {
