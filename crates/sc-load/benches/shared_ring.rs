...
impl BenchMode {
    fn from_environment() -> Self {
        match std::env::var("SC_LOAD_BENCH_MODE").as_deref() {
            Ok("shared") => Self::Shared,
            Ok("standard") => Self::Standard,
            Ok("both") | Err(_) => Self::Both,
            Ok(value) => {
                panic!("SC_LOAD_BENCH_MODE must be standard, shared, or both; got {value}")
            }
        }
    }
}
...
impl BenchConfig {
    fn from_environment() -> Self {
        Self {
            rows: environment_usize("SC_LOAD_BENCH_ROWS", DEFAULT_ROWS),
            columns: environment_usize("SC_LOAD_BENCH_COLUMNS", DEFAULT_COLUMNS),
            batch_size: environment_usize("SC_LOAD_BENCH_BATCH_SIZE", DEFAULT_BATCH_SIZE),
            prefetch_step: environment_usize("SC_LOAD_BENCH_PREFETCH_STEP", DEFAULT_PREFETCH_STEP),
            world_size: environment_usize("SC_LOAD_BENCH_WORLD_SIZE", DEFAULT_WORLD_SIZE),
            iterations: environment_usize("SC_LOAD_BENCH_ITERATIONS", DEFAULT_ITERATIONS),
...
fn make_plan(config: BenchConfig) -> (tempfile::TempDir, Plan) {
    ...
    DenseWriter::new(
        &path,
        Partition::fixed_cells(u64::try_from(config.rows).unwrap()),
