...
use crate::error::{from_rust, invalid_argument};
...
#[pyclass(name = "_Session", module = "sc_load._core")]
...
impl PySession {
    fn next_batch<'py>(&self, py: Python<'py>) -> PyResult<Option<Bound<'py, PyAny>>> {
        let batch = py.allow_threads(|| self.next_owned());
        match batch {
    ...
    fn close(&self, py: Python<'_>) {
        ...
        py.allow_threads(move || drop((session, cancellation)));
    }

    #[getter]
    fn closed(&self) -> bool {
        self.slot.lock().session.is_none()
    }

    #[getter]
    fn exhausted(&self) -> bool {
        self.slot.lock().exhausted
    }
