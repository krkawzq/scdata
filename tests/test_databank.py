...
def _array_meta(root: Path, key: str) -> ArrayMeta:
    ...
    locs = tuple(
        ChunkLocation(offset=o, length=l) for o, l in zip(sc["offsets"], sc["lengths"])
    )
...
def test_register_dense_2d(dense_store_factory) -> None:
    root = dense_store_factory("dense", (3, 4), (2, 2), np.float32, None,
                              ["g0", "g1", "g2", "g3"])
...
def test_dataset_id_identity(dense_store_factory) -> None:
    root = dense_store_factory("id", (3, 4), (2, 2), np.float32, None,
                              ["g0", "g1", "g2", "g3"])
    ds = _dense_dataset(root, ["g0", "g1", "g2", "g3"])
    bank = ScDataBank()
    did = bank.register_dense(ds, str(root))
...
def test_access_dense_values(dense_store_factory) -> None:
    root = dense_store_factory("acc", (5, 6), (2, 3), np.float32, None,
...
def test_access_dense_codec(codec_configs, dense_store_factory, codec_name) -> None:
    root = dense_store_factory(f"acc_{codec_name}", (5, 6), (2, 3), np.float64,
                              codec_configs[codec_name], [f"g{i}" for i in range(6)])
    ds = _dense_dataset(root, [f"g{i}" for i in range(6)])
    bank = ScDataBank()
    did = bank.register_dense(ds, str(root))
...
def test_access_dtype_round_trip(dense_store_factory) -> None:
    for np_dt in (np.int32, np.int64, np.uint8, np.float32, np.float64):
        root = dense_store_factory(f"dt_{np_dt.__name__}", (3, 4), (2, 2), np_dt,
                                  None, ["g0", "g1", "g2", "g3"])
        ds = _dense_dataset(root, ["g0", "g1", "g2", "g3"])
        bank = ScDataBank()
        did = bank.register_dense(ds, str(root))
...
def test_access_by_gene_names(dense_store_factory) -> None:
    root = dense_store_factory("gn", (5, 6), (2, 3), np.float32, None,
                              [f"g{i}" for i in range(6)])
    ds = _dense_dataset(root, [f"g{i}" for i in range(6)])
    bank = ScDataBank()
    did = bank.register_dense(ds, str(root))
...
def test_access_by_gene_names_missing_error(dense_store_factory) -> None:
    root = dense_store_factory("gn_err", (3, 4), (2, 2), np.float32, None,
                              ["g0", "g1", "g2", "g3"])
    ds = _dense_dataset(root, ["g0", "g1", "g2", "g3"])
    bank = ScDataBank()
    did = bank.register_dense(ds, str(root))
    with pytest.raises(DataBankError):
        bank.access_cells_by_gene_names(did, [0], ["nope"],
                                        missing=MissingGenePolicy.ERROR)
    bank.unregister(did)


def test_access_unregistered_raises(dense_store_factory) -> None:
    root = dense_store_factory("unreg", (3, 4), (2, 2), np.float32, None,
                              ["g0", "g1", "g2", "g3"])
    ds = _dense_dataset(root, ["g0", "g1", "g2", "g3"])
    bank = ScDataBank()
    did = bank.register_dense(ds, str(root))
...
def test_prefetch_cells(dense_store_factory) -> None:
    root = dense_store_factory("pf", (5, 6), (2, 3), np.float32, None,
                              [f"g{i}" for i in range(6)])
    ds = _dense_dataset(root, [f"g{i}" for i in range(6)])
    bank = ScDataBank()
    did = bank.register_dense(ds, str(root))
...
def test_prefetch_by_gene_names(dense_store_factory) -> None:
    root = dense_store_factory("pfgn", (5, 6), (2, 3), np.float32, None,
                              [f"g{i}" for i in range(6)])
    ds = _dense_dataset(root, [f"g{i}" for i in range(6)])
    bank = ScDataBank()
    did = bank.register_dense(ds, str(root))
...
def test_multiple_banks(dense_store_factory) -> None:
    r1 = dense_store_factory("b1", (3, 4), (2, 2), np.float32, None,
                             ["g0", "g1", "g2", "g3"])
    r2 = dense_store_factory("b2", (3, 4), (2, 2), np.float64, None,
                             ["h0", "h1", "h2", "h3"])
    b1 = ScDataBank()
    b2 = ScDataBank()
