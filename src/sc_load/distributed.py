...
from sc_load._validation import as_int, dtype_from_core
from sc_load.config import PlanConfig, SessionConfig
from sc_load.dataset import RowRef, ScDataset
from sc_load.exceptions import CancelledError, UnsupportedError, _call_core
from sc_load.output import OutputSpec
from sc_load.stats import SessionState

if TYPE_CHECKING:
    from sc_load.plan import Plan
...
class DistributedSession:
    ...
    def __init__(
        ...
        *,
        ...
        if not hasattr(_core, "_attach_shared") or not hasattr(plan._inner, "open_shared"):
    ...
    def state(self) -> SessionState:
        return self._runner.inner.state
    ...
    def _new_iterator(self, rank: int, copy: bool) -> DistributedIterator:
        descriptor = _call_core(self._runner.inner.duplicate_fd)
        try:
    ...
    def cancel(self) -> None:
        ...
        if not self._thread.is_alive():
            return
        _call_core(self._runner.inner.cancel)
...
class DistributedIterator(Iterator[NDArray[Any]]):
    ...
    def _next_batch(self, *, copy: bool | None) -> NDArray[Any] | None:
        ...
        try:
            batch = _call_core(client.next_batch_copy if resolved_copy else client.next_batch)
        except BaseException:
            self.close()
            raise
        if batch is None:
    ...
    def _read(self) -> NDArray[Any]:
        ...
        client = self._ensure_client()
        try:
            output, batches = _call_core(client.read, remaining)
        except BaseException:
            self.close()
            raise
    ...
    def _ensure_client(self) -> _core._SharedClient:
        ...
        with self._resource_lock:
            ...
            try:
                client = _call_core(_core._attach_shared, self._descriptor, self.rank)
            except BaseException:
...
def distributed_prefetch(
    ...
    *,
    ...
    from sc_load.plan import compile
