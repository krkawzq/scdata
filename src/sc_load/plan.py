...
from sc_load._validation import as_int, normalize_rows
from sc_load.config import PlanConfig, SessionConfig
from sc_load.dataset import RowRef, ScDataset
from sc_load.exceptions import InternalError, _call_core
from sc_load.output import OutputSpec
from sc_load.stats import PlanStats, RuntimeStats, SessionState

if TYPE_CHECKING:
    from sc_load.distributed import DistributedSession
...
class Plan:
    ...
    __slots__ = ("_inner", "_output", "_stats")

    def __init__(self, inner: _core._Plan, output: OutputSpec) -> None:
        self._inner = inner
        self._output = output
        self._stats = PlanStats._from_mapping(_call_core(inner.stats))
    ...
    def batch_count(self) -> int:
        return self._inner.batch_count

    @property
    def prefetch_step(self) -> int:
        return self._inner.prefetch_step
    ...
    def row_stride_bytes(self) -> int:
        return self._inner.row_stride_bytes
    ...
    def open(self, config: SessionConfig | None = None) -> Session:
        ...
        inner = _call_core(self._inner.open, resolved._to_core())
...
class Session(Iterator[NDArray[Any]]):
    ...
    def stats(self) -> RuntimeStats:
        return RuntimeStats._from_mapping(_call_core(self._inner.stats))
    ...
    def next_batch(self) -> NDArray[Any] | None:
        ...
        batch = _call_core(self._inner.next_batch)
...
def compile(
    datasets: ScDataset | Iterable[ScDataset],
    ...
    *,
    ...
    ``datasets`` is a single :class:`ScDataset` or a collection. Collection
...
def _normalize_datasets(datasets: ScDataset | Iterable[ScDataset]) -> list[ScDataset]:
    if isinstance(datasets, ScDataset):
        return [datasets]
    try:
        dataset_list = list(datasets)
    except TypeError as error:
        raise TypeError("datasets must be a ScDataset or iterable of ScDataset objects") from error
    for index, dataset in enumerate(dataset_list):
        if not isinstance(dataset, ScDataset):
            raise TypeError(f"datasets[{index}] must be a ScDataset instance")
    return dataset_list


def _infer_output(datasets: list[ScDataset]) -> OutputSpec:
...
def _validate_datasets_for_output(datasets: list[ScDataset], output: OutputSpec) -> None:
...
def _has_identity_feature_map(dataset: ScDataset) -> bool:
