"""ScDataset registration over AnnData ``.scc`` / ``.scc.zip`` matrices."""
...
from sc_load._validation import as_int, dtype_from_core, normalize_feature_map
from sc_load.config import ReadLimits
from sc_load.exceptions import _call_core
from sc_load.scc import normalize_matrix_key, read_feature_names, resolve_matrix_location

DatasetKind = Literal["dense", "csr"]

__all__ = ["DatasetKind", "RowRef", "ScDataset", "register"]


def _coerce_read_limits(limits: object | None) -> ReadLimits:
    """Accept ``sc_load.ReadLimits`` or duck-typed ``sc_compress.ReadLimits``."""
    if limits is None:
        return ReadLimits()
    ...
    try:
        ...
        raise TypeError("limits must be sc_load.ReadLimits or sc_compress.ReadLimits") from error
...
class ScDataset:
    ...
    def __init__(
        ...
        *,
        ...
        if feature_map is None:
            ...
            map_array = normalize_feature_map(feature_map, inner.n_cols)
            map_array.flags.writeable = False
            map_tuple = tuple(None if value == -1 else int(value) for value in map_array)

        if resolved_names is not None and len(resolved_names) != inner.n_cols:
            raise ValueError(
                f"feature_names has length {len(resolved_names)}, "
                f"but the matrix has {inner.n_cols} columns"
            )
    ...
    def dtype(self) -> np.dtype[Any]:
        return dtype_from_core(self._inner.dtype)
    ...
    def with_feature_map(self, feature_map: Iterable[int | None] | None) -> ScDataset:
    ...
    def __repr__(self) -> str:
        ...
        return (
            f"ScDataset(kind={self.kind!r}, shape={self.shape!r}, dtype={self.dtype.name!r}, "
...
def register(
    ...
    *,
    ...
    ``limits`` accepts :class:`sc_load.ReadLimits` or
    :class:`sc_compress.ReadLimits`.
    """
    return ScDataset(
        path,
        key=key,
