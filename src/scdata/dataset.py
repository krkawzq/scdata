...
from scdata._validation import as_int, dtype_from_core, normalize_feature_map
from scdata.config import ReadLimits
from scdata.exceptions import _call_core
from scdata.scc import normalize_matrix_key, read_feature_names, resolve_matrix_location
...
def _coerce_read_limits(limits: object | None) -> ReadLimits:
    """Accept ``scdata.ReadLimits`` or duck-typed ``sc_compress.ReadLimits``."""
    if limits is None:
        return ReadLimits()
    ...
    try:
        ...
        raise TypeError("limits must be scdata.ReadLimits or sc_compress.ReadLimits") from error
...
def register(
    ...
    *,
    ...
    ``limits`` accepts :class:`scdata.ReadLimits` or
