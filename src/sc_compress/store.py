...
from sc_compress._index import AxisSpec, normalize_key, rust_payload
from sc_compress._validate import as_int, normalize_row_range
from sc_compress.array import ScCsr, ScDense
from sc_compress.exceptions import _invalid_argument
from sc_compress.format import dtype_from_storage
from sc_compress.limits import ReadLimits
...
class Store:
    ...
    Construct via :func:`sc_compress.open_store`. Indexing materializes only the
    ...
    * fancy indices, bool masks, and strided slices are all supported
    ...
    __slots__ = ("_handle", "_path", "_zip_prefix")
    ...
    def __init__(
        ...
        if not isinstance(handle, _core._Store):
            raise TypeError("handle must be sc_compress._core._Store")
    ...
    def kind(self) -> Literal["dense", "csr"]:
        return self._require_open().kind

    @property
    def shape(self) -> tuple[int, int]:
        rows, columns = self._require_open().shape
    ...
    def storage_dtype(self) -> str:
        ...
        return self._require_open().value_dtype

    @property
    def storage_index_dtype(self) -> str | None:
        """On-disk CSR index dtype name, or ``None`` for dense stores."""
        return self._require_open().index_dtype

    @property
    def nnz(self) -> int | None:
        value = self._require_open().nnz
        return None if value is None else int(value)

    @property
    def n_workers(self) -> int:
        """Maximum chunk workers used by each decode call."""
        return int(self._require_open().n_workers)

    @property
    def limits(self) -> ReadLimits:
        handle = self._require_open()
        return ReadLimits(
            max_metadata_size=handle.maximum_metadata_size,
            max_encoded_size=handle.maximum_encoded_size,
            max_decoded_size=handle.maximum_decoded_size,
            max_block_count=handle.maximum_block_count,
            n_workers=handle.n_workers,
        )
    ...
    def indptr(self) -> NDArray[np.uint64] | None:
        ...
        values = self._require_open().indptr()
    ...
    def read_rows(
        ...
        *,
        ...
        from sc_compress._index import normalize_axis
    ...
    def _require_open(self) -> _core._Store:
        if self._handle is None:
            _invalid_argument("I/O operation on closed sc_compress.Store")
        return self._handle
